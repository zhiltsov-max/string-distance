...
pub fn levenstein_ascii(a: &String, b: &String) -> i32 {
...
pub fn levenstein_utf(a: &String, b: &String) -> i32 {
    // len + 1 is an extra row/col to avoid an extra loop after the main one
    // len + 2 is an extra row/col to avoid branching
    let row_size = b.len() + 2;
    let mut states: Vec<i32> = vec![
...
pub fn levenstein(a: &String, b: &String) -> i32 {
    if is_ascii(&a) && is_ascii(&b) {
        return levenstein_ascii(&a, &b);
    } else {
        return levenstein_utf(&a, &b);
    }
}
